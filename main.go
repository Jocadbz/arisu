...
func handleResponse(response string, client AIClient, config *Config) {
    if strings.Contains(response, "<EDIT>") && strings.Contains(response, "</EDIT>") {
        block := response[strings.Index(response, "<EDIT>")+len("<EDIT>") : strings.Index(response, "</EDIT>")]
        lines := strings.SplitN(strings.TrimSpace(block), "\n", 2)
        if len(lines) == 2 {
            filename := strings.TrimSpace(lines[0])
            content := lines[1]
            action := EditAction{Filename: filename, Content: content}
            _ = action.Execute(client, config)
        }
    }

    if strings.Contains(response, "<RUN>") && strings.Contains(response, "</RUN>") {
        cmd := response[strings.Index(response, "<RUN>")+len("<RUN>") : strings.Index(response, "</RUN>")]
        action := RunAction{Command: strings.TrimSpace(cmd)}
        _ = action.Execute(client, config)
    }

    if strings.Contains(response, "<READ>") && strings.Contains(response, "</READ>") {
        file := response[strings.Index(response, "<READ>")+len("<READ>") : strings.Index(response, "</READ>")]
        action := ReadAction{Filename: strings.TrimSpace(file)}
        _ = action.Execute(client, config)
    }
}
...
func runAgenticMode(agentFile string, client AIClient, config *Config, logDir string) {
    ...
    for _, step := range steps {
        ...
        // Handle <EDIT> and <RUN> tags in the step response
        handleResponse(response, client, config)
        ...
        // Send "Proceed." and handle its response
        ...
        // Handle <EDIT> and <RUN> tags in the proceed response
